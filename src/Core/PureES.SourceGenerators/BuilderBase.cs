...
internal abstract class BuilderBase
{
    ...
    /// </summary>
    /// <returns></returns>
    protected bool ValidateSingleAttribute(IMethod method, string attribute, out IParameter parameter)
    {
        var list = method.Parameters.Where(p => p.Attributes.Contains(attribute)).ToList();
        switch (list.Count)
        {
            case 0:
            ...
            case 1:
                ...
                return true;
            default:
                Log.MultipleParametersDefinedWithAttribute(method, attribute);
                parameter = null!;
                return false;
        }
    }
