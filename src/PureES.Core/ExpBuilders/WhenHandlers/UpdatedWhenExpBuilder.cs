...
public class UpdatedWhenExpBuilder
{
    ...
    public UpdatedWhenExpBuilder(CommandHandlerOptions options)
    {
        _options = options;
    }
    ...
    public void ValidateUpdatedWhen(Type aggregateType, MethodInfo method)
    {
        ...
        LoadExpBuilder.ValidateWhen(aggregateType, method);
        ...
        new LoadExpBuilder(_options).ValidateEnvelope(parameters[1]);
    }
    ...
    public bool IsUpdatedWhen(Type aggregateType, MethodInfo method)
    {
        ...
        return _options.IsEventEnvelope?.Invoke(method.GetParameters()[1].ParameterType)
               ?? new LoadExpBuilder(_options).IsEnvelope(method.GetParameters()[1].ParameterType);
    }
}