...
public class CreatedWhenCustomEnvelopeTests
{
    ...
    {
        var builder = new CreatedWhenExpBuilder(Options);
    ...
    public void InvokeCreatedWhen()
    {
        ...
        EventEnvelope CreateEnvelope(object @event) => new (Guid.NewGuid(),
            Guid.NewGuid().ToString(),
            Rand.NextULong(),
            DateTime.UtcNow,
            @event,
    ...
    private record Aggregate(EventEnvelope<Created1>? One, EventEnvelope<Created2>? Two)
    {
        public static Aggregate When(EventEnvelope<Created1> envelope) =>
            new(envelope, null);
        public static Aggregate When(EventEnvelope<Created2> envelope) =>
            new(null, envelope);

        public static Aggregate When(Aggregate _, EventEnvelope<Created3> __) =>
