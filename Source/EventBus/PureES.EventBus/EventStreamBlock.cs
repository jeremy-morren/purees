...
/// </summary>
internal sealed class EventStreamBlock : ITargetBlock<EventEnvelope>
{
    private readonly Dictionary<string, EventQueue> _queues = new();

    private readonly ITargetBlock<EventEnvelope> _target;

    public EventStreamBlock(Func<EventEnvelope, Task> action, EventBusOptions options)
    {
        ...
        var worker = new TransformBlock<EventQueue, string>(async queue =>
        {
            ...
            lock (queue)
            {
                if (queue.Mutex.CurrentCount == 0)
                    return queue.StreamId; //Another worker is operating on this stream below, we can skip to the end
            }
