...
internal class AggregateStore<TAggregate> : IAggregateStore<TAggregate> where TAggregate : notnull
{
    ...
    public ValueTask<LoadedAggregate<TAggregate>> Create(IAsyncEnumerable<EventEnvelope> events,
    ...
    public ValueTask<LoadedAggregate<TAggregate>> LoadPartial(string streamId, ulong requiredVersion,
        CancellationToken token)
