...
namespace PureES.CommandHandlers
{
    ...
    internal sealed class shortCommandHandler : global::PureES.ICommandHandler<short>
    {
        ...
        [global::System.Diagnostics.DebuggerStepThroughAttribute()]
        [global::System.Diagnostics.DebuggerNonUserCodeAttribute()]
        public string GetStreamId(short command)
        {
            if (command == null)
            {
        ...
        public async global::System.Threading.Tasks.Task<uint> Handle(short command, CancellationToken cancellationToken)
        {
            ...
            using (var activity = PureES.PureESTracing.ActivitySource.StartActivity("HandleCommand"))
            {
                if (activity != null)
                {
                    activity.DisplayName = "HandleCommand TestAggregate.TransactionValueTask";
                    if (activity.IsAllDataRequested)
                    {
                        activity?.SetTag("Command", CommandType);
                        activity?.SetTag("Aggregate", AggregateType);
                        activity?.SetTag("Method", "TransactionValueTask");
                    }
                }
